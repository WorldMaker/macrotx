...
var qs = (container2, selector) => {
  const node = container2.querySelector(selector);
  if (node) {
    return node;
  }
  throw new Error("Stamp does not match component");
};
function selectBindings(container2, description) {
  const { elementSelectors, nodeSelectors } = collectBindings(description);
  const elementBinds = elementSelectors.map(([selector, desc]) => [qs(container2, selector), desc]);
  const nodeBinds = nodeSelectors.map(([selector, desc]) => [
    qs(container2, selector),
    desc
  ]);
  return { container: container2, nodeBinds, elementBinds };
}
...
var StampCollection = class {
   ...
   */
  isPrestamp(c, properties, container2) {
    const stampApplies = this.#prestampMap.get(container2);
    if (stampApplies) {
   ...
   * @returns this (for chaining)
   */
  registerPrestamp(c, container2, when) {
    this.#prestampMap.set(container2, [c, when ?? (() => true)]);
    return this;
  }
};

// node_modules/butterfloat/wiring-dom-stamp.js
var stampOrBuildStrategy = (stamps2) => (component, properties, context2, container2, document2) => {
  if (container2 && stamps2.isPrestamp(component, properties, container2)) {
    return {
  ...
  if (stamp) {
    const container3 = stamp.content.cloneNode(true);
    return {
      ...selectBindings(container3, component(properties, context2)),
...
function runStamps(container2, component, stamps2, options, placeholder, document2 = globalThis.document) {
...
stamps.registerPrestamp(Main, container).registerOnlyStamp(
  PageNotFound,
  container.querySelector("#page-not-found")
).registerOnlyStamp(
  NavBar,
  container.querySelector("#navbar")
).registerOnlyStamp(
  HomePage,
  container.querySelector("#home-page")
).registerOnlyStamp(
  AboutPage,
  container.querySelector("#about-page")
).registerOnlyStamp(
  GamePage,
  container.querySelector("#game-page")
).registerOnlyStamp(
  NodeNotFound,
  container.querySelector("#node-not-found")
).registerOnlyStamp(
  GetStarted,
