...
function buildStamp(description, document2 = globalThis.document) {
  ...
  const { elementBinds, nodeBinds } = buildTree(description, template.content, void 0, void 0, void 0, document2);
  let i = 0;
...
function AfterTea({ game: game2 }) {
  const chefTea = game2.className.pipe(
    (0, import_rxjs5.map)((className) => className === "Chef" ? ChefTea : Empty2)
  );
...
function GamePage(_props, { bindEffect, events }) {
  ...
  const weapon = game2.weapon.pipe(
    (0, import_rxjs7.map)(
      (weapon2) => !weapon2 || weapon2 === "" ? Empty3 : () => /* @__PURE__ */ jsx(Weapon, { game: game2 })
    )
  );
